...
def deduplicate_bindings(bindings: frozenlist[hydra.core.Binding]) -> frozenlist[hydra.core.Binding]:
    ...
    return hydra.lib.lists.foldl((lambda acc, b: (n := b.name.value, used_names := hydra.lib.lists.map((lambda a: a.name.value), acc), unique_name := find_unique_name(n, used_names), hydra.lib.lists.concat2(acc, (hydra.core.Binding(hydra.core.Name(unique_name), b.term, b.type_scheme),)))[3]), (), bindings)
...
def dsl_module(cx: T0, graph: hydra.graph.Graph, mod: hydra.packaging.Module):
    def _hoist_hydra_dsls_dsl_module_1(v1):
        match v1:
            ...
            case _:
                return Nothing()
    return hydra.lib.eithers.bind(filter_type_bindings(cx, graph, hydra.lib.maybes.cat(hydra.lib.lists.map((lambda d: _hoist_hydra_dsls_dsl_module_1(d)), mod.definitions))), (lambda type_bindings: hydra.lib.logic.if_else(hydra.lib.lists.null(type_bindings), (lambda : Right(Nothing())), (lambda : hydra.lib.eithers.bind(hydra.lib.eithers.map_list((lambda b: hydra.lib.eithers.bimap((lambda _e: cast(hydra.errors.Error, hydra.errors.ErrorDecoding(_e))), (lambda x: x), generate_bindings_for_type(cx, graph, b))), type_bindings), (lambda dsl_bindings: Right(Just(hydra.packaging.Module(Just(hydra.lib.strings.cat(("DSL functions for ", mod.namespace.value))), dsl_namespace(mod.namespace), hydra.lib.lists.nub(hydra.lib.lists.map((lambda x1: dsl_namespace(x1)), mod.type_dependencies)), hydra.lib.lists.nub(hydra.lib.lists.concat2((mod.namespace, hydra.packaging.Namespace("hydra.phantoms")), mod.type_dependencies)), hydra.lib.lists.map((lambda b: cast(hydra.packaging.Definition, hydra.packaging.DefinitionTerm(hydra.packaging.TermDefinition(b.name, b.term, b.type_scheme)))), deduplicate_bindings(hydra.lib.lists.concat(dsl_bindings))))))))))))
