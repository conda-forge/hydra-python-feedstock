...
def encode_float(fv: hydra.core.FloatValue) -> Either[str, hydra.json.model.Value]:
    r"""Encode a float value to JSON. Finite values become JSON numbers (shortest round-trip); IEEE specials (NaN/Inf/-0.0) become JSON strings. Float32 and Float64 are symmetric; the schema disambiguates precision on decode. Bigfloat rejects anything the decimal space can't hold."""
    match fv:
        ...
        case hydra.core.FloatValueFloat32(value=f):
            @lru_cache(1)
            def s() -> str:
                return hydra.lib.literals.show_float32(f)
            return hydra.lib.logic.if_else(requires_json_string_sentinel(s()), (lambda : Right(cast(hydra.json.model.Value, hydra.json.model.ValueString(s())))), (lambda : Right(cast(hydra.json.model.Value, hydra.json.model.ValueNumber(hydra.lib.literals.float32_to_decimal(f))))))
