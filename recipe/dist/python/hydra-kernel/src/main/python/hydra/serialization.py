...
def custom_indent(idt: str, s: str) -> str:
    r"""Indent every non-empty line of `s` by `idt`. Empty lines stay empty (no trailing whitespace) so downstream byte-identity checks don't care about indent depth."""
    return hydra.lib.strings.cat(hydra.lib.lists.intersperse("\n", hydra.lib.lists.map((lambda line: hydra.lib.logic.if_else(hydra.lib.equality.equal(line, ""), (lambda : line), (lambda : hydra.lib.strings.cat2(idt, line)))), hydra.lib.strings.lines(s))))
...
def print_expr(e: hydra.ast.Expr) -> str:
    ...
    match e:
        ...
        case hydra.ast.ExprIndent(value=indent_expr):
            ...
            def indent_line(idt2: str, line: str) -> str:
                return hydra.lib.logic.if_else(hydra.lib.equality.equal(line, ""), (lambda : line), (lambda : hydra.lib.strings.cat2(idt2, line)))
            @lru_cache(1)
            def ilns():
                def _hoist_ilns_1(v1):
                    match v1:
                        case hydra.ast.IndentStyleAllLines(value=idt2):
                            return hydra.lib.lists.map((lambda v12: indent_line(idt2, v12)), lns())
                        case hydra.ast.IndentStyleSubsequentLines(value=idt2):
                            return hydra.lib.logic.if_else(hydra.lib.equality.equal(hydra.lib.lists.length(lns()), 1), (lambda : lns()), (lambda : hydra.lib.maybes.from_maybe((lambda : lns()), hydra.lib.maybes.map((lambda uc: hydra.lib.lists.cons(hydra.lib.pairs.first(uc), hydra.lib.lists.map((lambda v12: indent_line(idt2, v12)), hydra.lib.pairs.second(uc)))), hydra.lib.lists.uncons(lns())))))
                        case _:
                            raise AssertionError("Unreachable: all variants handled")
        ...
        case hydra.ast.ExprSeq(value=seq_expr):
            ...
            def is_newline_ws(ws: hydra.ast.Ws):
                def _hoist_is_newline_ws_1(v1):
                    match v1:
                        case hydra.ast.WsBreak():
                            return True
                        case hydra.ast.WsBreakAndIndent():
                        ...
                        case _:
                            return False
                return _hoist_is_newline_ws_1(ws)
            @lru_cache(1)
            def spadl_is_newline() -> bool:
                return is_newline_ws(spadl)
            @lru_cache(1)
            def spadr_is_newline() -> bool:
                return is_newline_ws(spadr)
            def join_elements(acc: str, el: str) -> str:
                @lru_cache(1)
                def el_starts_with_newline() -> bool:
                    return hydra.lib.maybes.maybe((lambda : False), (lambda c: hydra.lib.equality.equal(c, 10)), hydra.lib.strings.maybe_char_at(0, el))
                @lru_cache(1)
                def el_is_empty() -> bool:
                    return hydra.lib.equality.equal(el, "")
                @lru_cache(1)
                def padl_eff() -> str:
                    return hydra.lib.logic.if_else(hydra.lib.logic.or_(hydra.lib.logic.and_(el_starts_with_newline(), hydra.lib.logic.not_(spadl_is_newline())), hydra.lib.logic.and_(el_is_empty(), hydra.lib.equality.equal(ssym, ""))), (lambda : ""), (lambda : pad(spadl)))
                @lru_cache(1)
                def padr_eff() -> str:
                    return hydra.lib.logic.if_else(hydra.lib.logic.or_(hydra.lib.logic.and_(el_starts_with_newline(), hydra.lib.logic.not_(spadr_is_newline())), hydra.lib.logic.and_(el_is_empty(), hydra.lib.equality.equal(ssym, ""))), (lambda : ""), (lambda : pad(spadr)))
                return hydra.lib.strings.cat((acc, padl_eff(), ssym, padr_eff(), el))
            return hydra.lib.maybes.maybe((lambda : ""), (lambda h: hydra.lib.lists.foldl(join_elements, h, hydra.lib.lists.drop(1, printed_elements()))), hydra.lib.lists.maybe_head(printed_elements()))
        case hydra.ast.ExprOp(value=op_expr):
            op = op_expr.op
            sym = op.symbol.value
            padding = op.padding
            ...
            @lru_cache(1)
            def padr_is_newline():
                def _hoist_padr_is_newline_1(v1):
                    match v1:
                        case hydra.ast.WsBreak():
                            return True
                        case hydra.ast.WsBreakAndIndent():
                        ...
                        case _:
                            return False
                return _hoist_padr_is_newline_1(padr)
            @lru_cache(1)
            def padl_effective() -> str:
                return hydra.lib.logic.if_else(hydra.lib.logic.and_(hydra.lib.equality.equal(sym, ""), hydra.lib.logic.or_(padr_is_newline(), hydra.lib.equality.equal(rhs(), ""))), (lambda : ""), (lambda : pad(padl)))
            @lru_cache(1)
            def padr_pad() -> str:
                return pad(padr)
            @lru_cache(1)
            def rhs_starts_with_newline() -> bool:
                return hydra.lib.maybes.maybe((lambda : False), (lambda c: hydra.lib.equality.equal(c, 10)), hydra.lib.strings.maybe_char_at(0, rhs()))
            @lru_cache(1)
            def padr_effective() -> str:
                return hydra.lib.logic.if_else(hydra.lib.logic.and_(rhs_starts_with_newline(), hydra.lib.logic.not_(padr_is_newline())), (lambda : ""), (lambda : padr_pad()))
            return hydra.lib.strings.cat2(hydra.lib.strings.cat2(hydra.lib.strings.cat2(hydra.lib.strings.cat2(lhs(), padl_effective()), sym), padr_effective()), rhs())
        case hydra.ast.ExprBrackets(value=bracket_expr):
