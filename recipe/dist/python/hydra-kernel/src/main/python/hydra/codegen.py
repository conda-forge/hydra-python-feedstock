...
def build_schema_map(g: hydra.graph.Graph) -> FrozenDict[hydra.core.Name, hydra.core.Type]:
    ...
    return hydra.lib.maps.map((lambda ts: hydra.strip.deannotate_type(ts.body)), g.schema_types)
...
def modules_to_graph(bs_graph: hydra.graph.Graph, universe_modules: frozenlist[hydra.packaging.Module], modules: frozenlist[hydra.packaging.Module]) -> hydra.graph.Graph:
        ...
        def _hoist_data_elements_1(v1):
            match v1:
                case hydra.packaging.DefinitionTerm(value=td):
                    return Just(hydra.core.Binding(td.name, td.term, td.type_scheme))
                case _:
                    return Nothing()
    ...
    def universe_bound_types() -> FrozenDict[hydra.core.Name, hydra.core.TypeScheme]:
        return hydra.lib.maps.from_list(hydra.lib.maybes.cat(hydra.lib.lists.map((lambda b: hydra.lib.maybes.map((lambda ts: (b.name, ts)), b.type_scheme)), universe_data_elements())))
...
def format_primitive(prim: hydra.graph.Primitive) -> str:
    ...
    @lru_cache(1)
    def type_str() -> str:
        return hydra.show.core.type_scheme(prim.type_scheme)
...
def format_term_binding(binding: hydra.core.Binding) -> str:
    ...
    @lru_cache(1)
    def type_str() -> str:
        return hydra.lib.maybes.maybe((lambda : "?"), (lambda scheme: hydra.show.core.type_scheme(scheme)), binding.type_scheme)
    return hydra.lib.strings.cat2(hydra.lib.strings.cat2(hydra.lib.strings.cat2("  ", name), " : "), type_str())
...
def refresh_module(inferred_elements: frozenlist[hydra.core.Binding], m: hydra.packaging.Module):
    ...
    def _hoist_hydra_codegen_refresh_module_2(inferred_elements, v1):
        match v1:
            ...
            case hydra.packaging.DefinitionTerm(value=td):
                return hydra.lib.maybes.map((lambda b: cast(hydra.packaging.Definition, hydra.packaging.DefinitionTerm(hydra.packaging.TermDefinition(b.name, b.term, b.type_scheme)))), hydra.lib.lists.find((lambda b: hydra.lib.equality.equal(b.name, td.name)), inferred_elements))
            case _:
                raise AssertionError("Unreachable: all variants handled")
    return hydra.lib.logic.if_else(hydra.lib.logic.not_(hydra.lib.logic.not_(hydra.lib.lists.null(hydra.lib.maybes.cat(hydra.lib.lists.map((lambda d: _hoist_hydra_codegen_refresh_module_1(d)), m.definitions))))), (lambda : m), (lambda : hydra.packaging.Module(m.description, m.namespace, m.term_dependencies, m.type_dependencies, hydra.lib.maybes.cat(hydra.lib.lists.map((lambda d: _hoist_hydra_codegen_refresh_module_2(inferred_elements, d)), m.definitions)))))
...
def module_to_source_module(m: hydra.packaging.Module) -> hydra.packaging.Module:
    ...
    return hydra.packaging.Module(Just(hydra.lib.strings.cat2("Source module for ", m.namespace.value)), source_ns(), (mod_type_ns,), (mod_type_ns,), (module_def(),))
