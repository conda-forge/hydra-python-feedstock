...
def other_error(oe: hydra.errors.OtherError) -> str:
    ...
    return oe.value
def resolution_error(re: hydra.errors.ResolutionError) -> str:
    r"""Show a resolution error as a string, including the offending name or shape."""
    match re:
        case hydra.errors.ResolutionErrorNoSuchBinding(value=e):
            return hydra.lib.strings.cat2("no such binding: ", e.name.value)
        case hydra.errors.ResolutionErrorNoSuchPrimitive(value=e2):
            return hydra.lib.strings.cat2("no such primitive: ", e2.name.value)
        case hydra.errors.ResolutionErrorNoMatchingField(value=e3):
            return hydra.lib.strings.cat2("no matching field: ", e3.field_name.value)
        case hydra.errors.ResolutionErrorOther(value=e4):
            return hydra.lib.strings.cat2("resolution error: ", e4.value)
        case hydra.errors.ResolutionErrorUnexpectedShape(value=e5):
            return hydra.lib.strings.cat(("unexpected shape: expected ", e5.expected, " but got ", e5.actual))
        case _:
...
def error(e: hydra.errors.Error) -> str:
    ...
    match e:
        ...
        case hydra.errors.ErrorResolution(value=v16):
            return resolution_error(v16)
        case hydra.errors.ErrorUndefinedField(value=v17):
            return hydra.show.error.core.undefined_field_error(v17)
        case hydra.errors.ErrorUndefinedTermVariable(value=v18):
            return hydra.show.error.core.undefined_term_variable_error(v18)
        case hydra.errors.ErrorUntypedTermVariable(value=v19):
            return hydra.show.error.core.untyped_term_variable_error(v19)
        case hydra.errors.ErrorUnexpectedTermVariant(value=v110):
            return hydra.show.error.core.unexpected_term_variant_error(v110)
        case hydra.errors.ErrorUnexpectedTypeVariant(value=v111):
            return hydra.show.error.core.unexpected_type_variant_error(v111)
        case hydra.errors.ErrorUnification(value=v112):
            return unification_error(v112)
        case _:
            raise AssertionError("Unreachable: all variants handled")
